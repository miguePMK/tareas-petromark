...
- **Informes:** indicadores para auditoria. Administrador y editor.
  Deshabilitado por defecto (ver `SECCIONES` en `constantes.js`).
- **Bases, Usuarios, Categorias, Externos:** ABM, solo administrador.

### Secciones que se pueden apagar

`SECCIONES` en `js/constantes.js` habilita o deshabilita partes del sistema sin
tocar el resto del codigo. Hoy `informes` esta en `false`: no aparece en el
menu y la ruta queda bloqueada aunque se la invoque a mano. Cambiar a `true`
para habilitarla.
